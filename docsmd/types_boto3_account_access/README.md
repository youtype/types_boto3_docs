...
Install `types-boto3` for `AccountAccess` service.

```bash
# install with boto3 type annotations
python -m pip install 'types-boto3[account-access]'

# Lite version does not provide session.client/resource overloads
# it is more RAM-friendly, but requires explicit type annotations
python -m pip install 'types-boto3-lite[account-access]'
...
```bash
python -m pip uninstall -y types-boto3-account-access
```
